...
waveRouter.post('/waves/:transform', bearerAuth, upload.any(), (request, response, next) => {
  if (request.files.length > 1 || request.files[0].fieldname !== 'wave' || !request.params.transform) {
  ...
  return Wave.findOne({ user: request.user._id }).then(wave => {
    if (wave) {
      ...
      return s3
        .remove(oldKey)
        .then(() => {
          return Wave.findOneAndRemove({ user: request.user._id }).then(() => {
            return fsx.readFile(file.path).then(data => {
              const parsedFile = waveParser(data);
              transformedFile = transformFunc(parsedFile);
              return fsx.writeFile(tempFilePath, transformedFile).then(() => {
